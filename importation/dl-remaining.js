...
cantons.features.forEach(canton => {
  ...
  if (props.Nom_abbr !== 'BE') {
    console.log('Skip', props.Nom_abbr);
    return;
  }
  let o = '';
  let coordinates = canton.geometry.coordinates[0][0];
  ...
  coordinates.forEach(c => {
    ...
    coordinatesReal.push([parseFloat(cc[0]), parseFloat(cc[1])]);
    o += cc[0] + ' ' + cc[1] + ' ';
  });
  rect = rect.map(l => { return l.reverse() });
  console.log(coordinatesReal);
  console.log(rect);
  let extractedPolygon = greinerHormann.intersection(coordinatesReal, rect.slice(0, -1));
  console.log(extractedPolygon);
  // return;

  o = '';
  extractedPolygon[0].forEach(c => {
    o += c[0] + ' ' + c[1] + ' ';
  });
  console.log(o);
  // return;
...
<osm-script output="json">
  ...
  cm += ' > buildings/' + props.Nom_abbr + '.4.gz';
  console.log(cm);
